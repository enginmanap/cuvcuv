...
bool ModelReader::readFace(std::stringstream& ss, int params[],
		int& readNumbers) {
	...
	for (int param = 0; param < MAX_PARAMS; ++param) {
		...
		if (endPos == 0) {
			std::cout << "face definition does not have a vertex index"
					<< std::endl;
			exit(1);
		}
		...
		while (endPos <= current.size()) {
			...
			//std::cout <<"start pos" << startPos << ", endPos " << endPos << std::endl;
			params[currentValue++] = atoi(
					current.substr(startPos, endPos - startPos).c_str());
		}
	}
	return true;
}
...
Model* ModelReader::readModelFile(Scene& scene) {
	...
	while (readLine(line)) {
		...
		if (command == "VertexCount") {
			...
			if (readFloatParams(stringStream, parameters, parameterCount)) {
				if (parameterCount < 3) {
					std::cerr << "vertex does not contain 3 coordinates, only "
							<< parameterCount << " provided." << std::endl;
				} else {
					model->addVertex(parameters[0], parameters[1],
							parameters[2]);
				}
			}
		} else if (command == "vt") {
			if (readFloatParams(stringStream, parameters, parameterCount)) {
				if (parameterCount < 2) {
					std::cerr
							<< "vertex texture coordinate does not contain 2 coordinates, only "
							<< parameterCount << " provided." << std::endl;
				} else {
					model->addVertexTextureCoordinate(parameters[0],
							parameters[1]);
				}
			}
		} else if (command == "vn") {
			if (readFloatParams(stringStream, parameters, parameterCount)) {
				if (parameterCount < 3) {
					std::cerr
							<< "vertex normal does not contain 3 coordinates, only "
							<< parameterCount << " provided." << std::endl;
				} else {
					model->addVertexNormal(parameters[0], parameters[1],
							parameters[2]);
				}
			}
		} else if (command == "s") {
			...
			if (readFace(stringStream, faceParameters, parameterCount)) {
				if (parameterCount < 3) {
					...
					if (faceParameters[1] == 0 && faceParameters[2] == 0) { //this is a basic triangle with out any normal definition
						...
						model->addTriangle(faceParameters[0],
								faceParameters[1 * 3], faceParameters[2 * 3],
								faceParameters[2], faceParameters[1 * 3 + 2],
								faceParameters[2 * 3 + 2], faceParameters[1],
								faceParameters[1 * 3 + 1],
								faceParameters[2 * 3 + 1]);
						//there might be more faces, we will interpret as GL_TRIANGLE_FAN
						int fanCenter = faceParameters[0], fanCenterNormal =
								faceParameters[2], fanCenterTexture =
								faceParameters[1], previousVertex =
								faceParameters[2 * 3], previousVertexNormal =
								faceParameters[2 * 3 + 2],
								previousVertexTexture =
										faceParameters[2 * 3 + 1];
						for (int i = 3; i < parameterCount; ++i) {
							model->addTriangle(fanCenter, previousVertex,
									faceParameters[i * 3], fanCenterNormal,
									previousVertexNormal,
									faceParameters[i * 3 + 2], fanCenterTexture,
									previousVertexTexture,
									faceParameters[i * 3 + 1]);
							previousVertex = faceParameters[i * 3];
			...
			if (readStringParams(stringStream, stringParams, 1)) {
				...
				if (materials.size() != 0) {
					...
					std::cerr << "Material file " << stringParams[0]
							<< " has no material to be read." << std::endl;
				}
			...
			if (readStringParams(stringStream, stringParams, 1)) {
				...
				if (mat != NULL) {
					...
					std::cerr << "Material with name " << stringParams[0]
